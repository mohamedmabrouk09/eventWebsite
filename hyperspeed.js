...
const lerp = (c, t, s = 0.1, l = 0.001) => {
  let ch = (t - c) * s;
  if (Math.abs(ch) < l) ch = t - c;
  return ch;
};
const rand = (b) =>
  Array.isArray(b) ? Math.random() * (b[1] - b[0]) + b[0] : Math.random() * b;
const pick = (a) =>
...
const turbDistortion = {
  ...
  `,
  getJS: (prog, t) => {
    const f = turbUni.uFreq.value,
      a = turbUni.uAmp.value;
    const gx = (p) =>
      Math.cos(Math.PI * p * f.x + t) * a.x +
      Math.pow(Math.cos(Math.PI * p * f.y + t * (f.y / f.x)), 2) * a.y;
    const gy = (p) =>
      -nsin(Math.PI * p * f.z + t) * a.z -
      Math.pow(nsin(Math.PI * p * f.w + t / (f.z / f.w)), 5) * a.w;
    return new THREE.Vector3(
      gx(prog) - gx(prog + 0.007),
      gy(prog) - gy(prog + 0.007),
      0,
    )
...
class Road {
  ...
  createPlane(side, isRoad) {
    ...
    let uni = {
      ...
      uColor: {
        value: new THREE.Color(
          isRoad ? o.colors.roadColor : o.colors.islandColor,
        ),
      },
      uTime: this.uTime,
      ...this.app.fogU,
    ...
    if (isRoad)
      Object.assign(uni, {
        ...
        uShoulderLinesWidthPercentage: {
          value: o.shoulderLinesWidthPercentage,
        },
    ...
    const mat = makeMat(
      roadVertex,
      isRoad ? roadFragment : islandFragment,
      uni,
      o.distortion,
...
class CarLights {
  ...
  init() {
    ...
    const aO = [],
      aM = [],
      aC = [];
    let cols = Array.isArray(this.colors)
      ? this.colors.map((c) => new THREE.Color(c))
      : new THREE.Color(this.colors);

    for (let i = 0; i < o.lightPairsPerRoadWay; i++) {
      const r = rand(o.carLightsRadius),
        len = rand(o.carLightsLength),
        spd = rand(this.speed);
      let laneX =
        (i % o.lanesPerRoad) * laneWidth -
        o.roadWidth / 2 +
        laneWidth / 2 +
        rand(o.carShiftX) * laneWidth;
    ...
    ig.setAttribute(
      "aOffset",
      new THREE.InstancedBufferAttribute(new Float32Array(aO), 3, false),
    );
    ig.setAttribute(
      "aMetrics",
      new THREE.InstancedBufferAttribute(new Float32Array(aM), 3, false),
    );
    ig.setAttribute(
      "aColor",
      new THREE.InstancedBufferAttribute(new Float32Array(aC), 3, false),
    );
...
class LightSticks {
  ...
  init() {
    const o = this.o,
      total = o.totalSideLightSticks;
    const stickStep = o.length / (total - 1);
    const ig = new THREE.InstancedBufferGeometry().copy(
    ...
    const aO = [],
      aC = [],
      aM = [];
    let cols = Array.isArray(o.colors.sticks)
      ? o.colors.sticks.map((c) => new THREE.Color(c))
    ...
    ig.setAttribute(
      "aOffset",
      new THREE.InstancedBufferAttribute(new Float32Array(aO), 1, false),
    );
    ig.setAttribute(
      "aColor",
      new THREE.InstancedBufferAttribute(new Float32Array(aC), 3, false),
    ...
    ig.setAttribute(
      "aMetrics",
      new THREE.InstancedBufferAttribute(new Float32Array(aM), 2, false),
    );
    ...
    const mat = makeMat(sticksVertex, sticksFragment, uni, o.distortion, {
      side: THREE.DoubleSide,
    });
    this.mesh = new THREE.Mesh(ig, mat);
    this.mesh.frustumCulled = false;
...
class HyperspeedApp {
  constructor(container, opts) {
    ...
    this.renderer.setPixelRatio(
      Math.min(devicePixelRatio, opts.maxPixelRatio || 2),
    );
    ...
    this.rCars = new CarLights(
      this,
      opts,
      opts.colors.rightCars,
      opts.movingCloserSpeed,
      new THREE.Vector2(1, 0 + opts.carLightsFade),
    );
    ...
    this._down = () => {
      this.fovTarget = opts.fovSpeedUp;
      this.speedUpTarget = opts.speedUp;
    };
    this._up = () => {
      this.fovTarget = opts.fov;
      this.speedUpTarget = 0;
  ...
  _resize() {
    ...
    this.renderer.setSize(W, H, false);
    this.renderer.setPixelRatio(
      Math.min(devicePixelRatio, this.opts.maxPixelRatio || 2),
    );
  ...
  init() {
    this.road.init();
    this.lCars.init();
    this.lCars.mesh.position.x =
      -this.opts.roadWidth / 2 - this.opts.islandWidth / 2;
    this.rCars.init();
    this.rCars.mesh.position.x =
      this.opts.roadWidth / 2 + this.opts.islandWidth / 2;
    this.sticks.init();
    this.sticks.mesh.position.x = -(
      this.opts.roadWidth +
      this.opts.islandWidth / 2
    );
    this._tick();
  }
...
export function createHyperspeed(container, overrides = {}) {
  ...
  const opts = {
    ...DEFAULT,
    ...overrides,
    colors: { ...DEFAULT.colors, ...(overrides.colors || {}) },
  };
